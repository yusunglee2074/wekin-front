<template>
  <div class="ui segment feed-list">
    ...
    <div class="action-container" v-if="likes">
      ...
      <span v-if="feed.user_key === (user ? user.user_key : 0)" style="float:right;cursor:pointer;" @click="onDeleteClick"><i class="erase icon"></i>삭제</span>
      <span v-if="feed.user_key === (user ? user.user_key : 0)" style="float:right;cursor:pointer;" @click="onModifyClick"><i class="write icon"></i>수정 </span> 
    </div>
